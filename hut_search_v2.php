<?php
// Alpine Hut Finder v2 — list huts by elevation with optional date-range availability
...
define('AVAIL_PAUSE_SEC', 0.5);   // seconds between availability API calls
...
function load_and_filter_huts(
    ...
    while (($row = fgetcsv($fh)) !== false) {
        ...
        $huts[] = [
            ...
            'availability'         => [],  // filled in later if dates provided
        ];
    }
    fclose($fh);
...
/**
 * Fetch availability for a single hut from hut-reservation.org.
 * Returns array keyed by 'YYYY-MM-DD' => ['freeBeds' => int, 'hutStatus' => str, 'totalSleepingPlaces' => int]
 * Returns [] on any error (silently — don't break the page).
 */
function fetch_availability(string $reservation_id, array $date_range): array {
    $url = AVAIL_URL . '?hutId=' . urlencode($reservation_id) . '&step=WIZARD';
...
/**
 * Fetch availability for all huts that have a reservation_id, with 0.5s pause between calls.
 */
function fetch_all_availability(array &$huts, array $date_range): void {
    ...
    foreach ($huts as &$hut) {
        ...
        if (!$first) {
            usleep((int)(AVAIL_PAUSE_SEC * 1_000_000));
        }
        $first = false;
...
function render_results(
    ...
    float   $distance_km,
    ?float  $min_elevation,
    array   $date_range        // empty if no dates given
): void {
    $total  = count($huts);
    $dn     = h($display_name);
    ...
    if ($with_avail) {
        $n = count($date_range);
        echo " &mdash; showing availability for <strong>$n</strong> day(s)";
    }
    echo "</div>\n";

    $day_labels = array_map(fn($d) => date('D d.m', strtotime($d)), $date_range);
    ...
    foreach ($huts as $i => $hut) {
        ...
        if ($with_avail) {
            ...
            foreach ($date_range as $date) {
                if (!$has_res) {
                    // No reservation system — span both columns
                    echo '<td class="avail-none" colspan="2">—</td>';
                    continue;
                }

                if (!isset($avail[$date])) {
                    // Has reservation_id but no data returned for this date
...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .searching { background: var(--yellow-bg); color: var(--yellow); border: 1px solid #ffeeba;
...
<header>
  ...
  <p>List huts near a location, sorted by elevation — with optional bed availability</p>
</header>
...
if ($location !== null && $location !== '') {
    ...
    if ($distance_km <= 0 || $distance_km > 500) {
        ...
        if (!empty($date_range)) {
            echo '<div class="searching" id="wait-msg">&#8987; Fetching availability data&hellip; please wait.</div>';
            flush();
        }
